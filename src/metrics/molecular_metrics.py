...
class SamplingMolecularMetrics(nn.Module):
    ...
    def forward(self, molecules: list, name, current_epoch, val_counter, local_rank, test=False, extra_name=''):
        stage = 'test' + extra_name if test else 'valid' + extra_name
        stability, rdkit_metrics, all_smiles = compute_molecular_metrics(molecules, self.train_smiles, self.dataset_info, stage)
        if test and local_rank == 0:
            with open(f'{current_epoch}_final_smiles.txt', 'w') as fp:
        ...
        for i, atom_type in enumerate(self.dataset_info.atom_decoder):
            ...
            to_log[f'molecular_metrics/{stage}/{atom_type}_dist'] = (generated_probability - target_probability).item()
        ...
        for j, bond_type in enumerate(['No bond', 'Single', 'Double', 'Triple', 'Aromatic']):
            ...
            to_log[f'molecular_metrics/{stage}/bond_{bond_type}_dist'] = (generated_probability - target_probability).item()
        ...
        for valency in range(6):
            ...
            to_log[f'molecular_metrics/{stage}/valency_{valency}_dist'] = (generated_probability - target_probability).item()
        ...
        if wandb.run:
            ...
            wandb.log({f'basic_metrics/{stage}/n_mae': n_mae,
                       f'basic_metrics/{stage}/node_mae': node_mae,
                       f'basic_metrics/{stage}/edge_mae': edge_mae,
                       f'basic_metrics/{stage}/valency_mae': valency_mae}, commit=False)
        ...
        if local_rank == 0:
            ...
            folder_path = f'graphs/{name}/{stage}'
            os.makedirs(folder_path, exist_ok=True)
            textfile = open(f'graphs/{name}/{stage}/valid_unique_molecules_e{current_epoch}_b{val_counter}.txt', "w")
            textfile.writelines(valid_unique_molecules)
            textfile.close()
            print("Stability metrics:", stability, "--", rdkit_metrics[0])

        return {'Validity': rdkit_metrics[0][0],}
