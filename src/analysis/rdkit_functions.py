...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info, stage='valid'):
    """ molecule_list: (dict) """
    ...
    if not dataset_info.remove_h:
        ...
        validity_dict = {f'{stage}/mol_stable': fraction_mol_stable, f'{stage}/atm_stable': fraction_atm_stable}
        if wandb.run:
            epoch = wandb.run.summary.get("epoch", 0)
            validity_dict.update({"epoch": epoch})
            wandb.log(validity_dict)
    else:
        validity_dict = {f'{stage}/mol_stable': -1, f'{stage}/atm_stable': -1}
    ...
    if wandb.run:
        nc = rdkit_metrics[-2]
        dic = {f'{stage}/Validity': rdkit_metrics[0][0], f'{stage}/Relaxed Validity': rdkit_metrics[0][1],
               f'{stage}/Uniqueness': rdkit_metrics[0][2], f'{stage}/Novelty': rdkit_metrics[0][3],
               f'{stage}/nc_max': nc['nc_max'], f'{stage}/nc_mu': nc['nc_mu']}
        epoch = wandb.run.summary.get("epoch", 0)
