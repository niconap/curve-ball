...
class TimedPoincareTransformer(nn.Module):
    def __init__(self, cfg, manifold, in_channels, num_layers, num_heads, dropout, max_seq_len, use_hyperbolic_attention, attention_type, attention_activation):
        ...
        # EDIT  
        # Optional scalar log-sigma^2 head for RG-VFM.
        # Operates on the tangent-at-0 representation and returns one scalar per node.
        self.predict_sigma = False
        try:
            self.predict_sigma = bool(getattr(cfg.model, "predict_sigma", False))
        except Exception:
            self.predict_sigma = False
    ...
    def forward(self, t, x, mask=None):
        ...
        Returns:
            mu: point on the Poincaré ball of shape [B, N, D].
            If predict_sigma=True, also returns log_sigma2 of shape [B, N].
        """
        # Get time embedding
        ...
        mu = self.output_projection(x)
        # EDIT
        if self.predict_sigma:
            # Scalar log-sigma^2 per node, computed in tangent-at-origin coords.
            mu_tangent = self.manifold.logmap0(mu)
            log_sigma2 = self.sigma_head(mu_tangent).squeeze(-1)
