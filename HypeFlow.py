...
class ManifoldFMLitModule(pl.LightningModule):
    ...
    def rfm_loss_fn(self, batch: torch.Tensor):
        ...
        if isinstance(batch, dict):
            ...
            # import pdb; pdb.set_trace()
            # EDIT 
            # RG-VFM: model may return (mu, log_sigma2) when predict_sigma=True.
            model_out = self.vecfield(t, x_t, mask=node_mask)
            if isinstance(model_out, tuple):
                x1_pred, log_sigma2 = model_out
            else:
                x1_pred, log_sigma2 = model_out, None
            # pdb.set_trace()
            ...
            # EDIT 
            # RG-VFM
            # Per-node neg log likelihood under a Riemannian Gaussian with mean mu=x1_pred and scalar variance sigma^2:
            #   neg log likelihood = 0.5 * d_g(mu, x1)^2 / sigma^2 + 0.5 * dim * log sigma^2
            # When predict_sigma=False (log_sigma2 is None), this collapses to the x1_prediction MSE-in-dist form.
                ...
                if log_sigma2 is None:
                    ...
                    log_s2 = log_sigma2.clamp(min=-6.0, max=4.0)
                    inv_s2 = torch.exp(-log_s2)
                    nll = 0.5 * d2 * inv_s2 + 0.5 * dim * log_s2
                    loss = (nll * node_mask).sum() / node_mask.sum()
