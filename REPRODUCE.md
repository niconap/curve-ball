...
## Post-training test metrics for (H)VQVAE on graph datasets

For `comm20`, `ego_small`, `planar`, `sbm`, and `tree`, you can evaluate degree,
clustering, and orbit against the **test split** from a trained checkpoint:

```bash
python3 src/evaluate_vqvae_test_metrics.py \
  +experiment=<experiment_name> \
  dataset=<dataset_name> \
  loss=VQVAE \
  general.test_only=<path_to_checkpoint> \
...
```bash
bash src/script/vqvae_test_metrics.sh <experiment_name> <dataset_name> <path_to_checkpoint>
```
